...
class InputProcessor extends Component {
    ...
    touchEndHandler(evt) {
        let cocosPos = evt.getLocation();
        let ti = this.updateInfo(evt.getID(), v2(cocosPos));
        evt.preventSwallow = (ti.target == this._owner);
        this.setEnd(ti);
        let evt2 = this.getEvent(ti, ti.target, Event.TOUCH_END, false, cocosPos);
        let cnt = ti.touchMonitors.length;
        for (let i = 0; i < cnt; i++) {
            let mm = ti.touchMonitors[i];
    ...
    getEvent(ti, target, type, bubbles, cocosPos) {
...
class GComponent extends GObject {
    ...
    onMaskReady() {
        ...
        if (this._maskContent instanceof GImage) {
            ...
            this._customMask.node.getComponent(Sprite).spriteFrame = this._maskContent._content.spriteFrame;
        }
...
class GRoot extends GComponent {
    ...
    static create(uiRoot) {
        GRoot._inst = new GRoot();
        uiRoot.addChild(GRoot._inst.node);
...
class GTextInput extends GTextField {
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (this._editBox.placeholderLabel) {
            ...
            this._editBox.placeholderLabel.getComponent(UITransform).anchorPoint = v2(0, 1);
            this._editBox.placeholderLabel.overflow = Label.Overflow.CLAMP;
            this._editBox.textLabel.getComponent(UITransform).anchorPoint = v2(0, 1);
            this._editBox.textLabel.overflow = Label.Overflow.CLAMP;
        }
    }
}
